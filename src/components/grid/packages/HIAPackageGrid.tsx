...
const HIAPackageGrid = ({ data }: { data: any[]; isUpdating: boolean }) => {
  // const navigate = useNavigate();
  const [showModal, setShowModal] = useState(false);
  // const [currentRowId, setCurrentRowId] = useState(null);
  // const [, setCurrentRowData] = useState({
  //   packageId: "",
  //   appId: "",
  // });
  ...
  const columns = [
    ...
    columnHelper.accessor((row: any) => row?.service, {
      id: "service",
      cell: (info) => (
        <div className="w-60 mx-auto text-left"> {info.getValue().name}</div>
      ),
      header: () => <div className="w-60 text-left">Service</div>,
    }),
